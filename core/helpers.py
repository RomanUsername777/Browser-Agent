...
class SignalHandler:
	...
	def wait_for_resume(self) -> None:
		...
		try:  # escape code is to blink the ...
			print(
				f'➡️  Press {green_color}[Enter]{reset_code} to resume or {red_color}[Ctrl+C]{reset_code} again to exit{blink_code}...{unblink_code} ',
				end='',
				flush=True,
				file=stderr,
			)
	...
	def decorator(func: Callable[P, R]) -> Callable[P, R]:
		...
		def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
			...
			if elapsed_time > 0.25:
				...
				log_instance.debug(f'⏳ {additional_text.strip("-")}() took {elapsed_time:.2f}s')
	...
	try:
		...
		if '*' in pattern_domain:
			...
			if pattern_domain.count('*.') > 1 or pattern_domain.count('.*') > 1:
				if log_warnings:
					log_instance = logging.getLogger(__name__)
					log_instance.error(f'⛔️ Multiple wildcards in pattern=[{normalized_pattern}] are not supported')
			...
			if pattern_domain.endswith('.*'):
				if log_warnings:
					log_instance = logging.getLogger(__name__)
					log_instance.error(f'⛔️ Wildcard TLDs like in pattern=[{normalized_pattern}] are not supported for security')
				return False  # Don't match unsafe patterns
			...
			if '*' in domain_without_wildcard:
				if log_warnings:
					log_instance = logging.getLogger(__name__)
					log_instance.error(f'⛔️ Only *.domain style patterns are supported, ignoring pattern=[{normalized_pattern}]')
				return False  # Don't match unsafe patterns
		...
		log_instance = logging.getLogger(__name__)
		log_instance.error(f'⛔️ Error matching URL {url} with pattern {domain_pattern}: {type(e).__name__}: {e}')
		return False
