...
class StepManager:
    ...
    def _get_page_state_attr(self, page_state: 'BrowserStateSummary | dict', attr: str, default: Any = None) -> Any:
        ...
        try:
            ...
            import traceback
            self.orchestrator.logger.error(f'Ошибка доступа к атрибуту {attr} в page_state (тип: {type(page_state)}): {e}')
            self.orchestrator.logger.error(f'Traceback: {traceback.format_exc()}')
            return default

    # ========== ОРКЕСТРАЦИЯ ШАГОВ ==========

    async def build_step_context(self, step_info: 'StepContext | None' = None) -> 'BrowserStateSummary':
    ...
    async def fetch_and_log_page_state(self) -> 'BrowserStateSummary':
        """Получает состояние страузера и логирует базовую информацию"""
        self.orchestrator.logger.debug(f'🌐 Шаг {self.orchestrator.state.n_steps}: Собираю состояние браузера...')
        t1 = time.time()
        page_state = await self.orchestrator.browser_session.get_browser_state_summary(
            include_screenshot=True,  # всегда делаем скриншот для всех шагов
            include_recent_events=self.orchestrator.include_recent_events,
        )
        ...
        await self.orchestrator._check_and_update_downloads(f'Шаг {self.orchestrator.state.n_steps}: после получения состояния браузера')
    ...
    def log_page_basic_info(self, page_state: 'BrowserStateSummary') -> None:
        ...
        self.orchestrator.logger.info(f'URL страницы: {url}')
        self.orchestrator.logger.info(f'Title страницы: {title}')
        
        # Логирование информации о скриншоте
        screenshot = self._get_page_state_attr(page_state, 'screenshot')
        if screenshot:
            self.orchestrator.logger.debug(f'📸 Скриншот получен, размер: {len(screenshot)} байт')
        else:
            self.orchestrator.logger.debug('📸 Скриншот не получен')
    ...
    async def analyze_page_elements(self, page_state: 'BrowserStateSummary') -> None:
        ...
        if not selector_map:
            return
        self.orchestrator.logger.info(f'Агент видит {len(selector_map)} интерактивных элементов на странице')
        
        # Логирование превью элементов (только первые 10)
        elements_preview = []
        ...
        if elements_preview:
            self.orchestrator.logger.info(f'Первые элементы которые видит агент:\n' + '\n'.join(elements_preview))
    ...
    async def handle_email_client_context(self, page_state: 'BrowserStateSummary') -> None:
        ...
        if not self.orchestrator.email_subagent.is_email_client(page_state):
            return
        
        # Извлекаем метаданные письма для логирования
        email_metadata = self.orchestrator.email_subagent.extract_email_metadata(page_state)
        if email_metadata['is_opened']:
            self.log_email_metadata(email_metadata)
        
        # Проверяем наличие диалогов через субагента
        if self.orchestrator.email_subagent.detect_dialog(page_state):
            self.orchestrator.logger.warning('Обнаружен открытый диалог на странице почтового клиента - он будет автоматически закрыт при следующем действии')

    def log_email_metadata(self, email_metadata: dict) -> None:
        """Логирует метаданные письма"""
        self.orchestrator.logger.info('Открыто письмо в почтовом клиенте')
        if email_metadata.get('subject'):
            self.orchestrator.logger.info(f'   Тема: {email_metadata["subject"]}')
        if email_metadata.get('sender'):
            self.orchestrator.logger.info(f'   Отправитель: {email_metadata["sender"]}')
        if email_metadata.get('body_preview'):
            ...
            self.orchestrator.logger.info(f'   Содержание (первые 200 символов): {body_preview}')

    async def prepare_actions_and_messages(self, page_state: 'BrowserStateSummary', step_info: 'StepContext | None') -> None:
        ...
        page_filtered_actions = self.orchestrator.tools.registry.get_prompt_description(page_url)

        # Логирование контекста шага
        self.orchestrator._log_step_context(page_state)
    ...
    async def update_page_action_models(self, page_url: str) -> None:
        ...
        self.orchestrator.logger.debug(f'📝 Шаг {self.orchestrator.state.n_steps}: Обновление моделей действий для страницы...')
        await self.orchestrator._update_action_models_for_page(page_url)

    async def check_forced_completion(self, step_info: 'StepContext | None') -> None:
        """Проверяет условия принудительного завершения"""
        await self.orchestrator._force_done_after_last_step(step_info)
        await self.orchestrator._force_done_after_failure()

    async def create_state_messages(self, page_state: 'BrowserStateSummary', step_info: 'StepContext | None', page_filtered_actions: str | None) -> None:
        """Создает сообщения состояния для LLM"""
        self.orchestrator.logger.debug(f'💬 Шаг {self.orchestrator.state.n_steps}: Формирование сообщений состояния...')
        unavailable_skills_info = None

        # Получение последнего ответа LLM и результатов действий
        agent_decision = self.orchestrator.state.last_model_output
        previous_action_results = self.orchestrator.state.last_result

        self.orchestrator._message_manager.create_state_messages(
            browser_state_summary=page_state,
            model_output=agent_decision,
            result=previous_action_results,
            step_info=step_info,
            use_vision=self.orchestrator.settings.use_vision,
            ...
            email_subagent=self.orchestrator.email_subagent,
        )
    ...
    async def apply_agent_actions(self) -> None:
        ...
        # Выполнение действий
        action_results = await self.orchestrator.multi_act(actions)
        
        # Сохранение результата выполнения действий
        self.orchestrator.state.last_result = action_results

    def has_model_output(self) -> bool:
        """Проверяет наличие вывода модели"""
        return self.orchestrator.state.last_model_output is not None

    def extract_actions_from_output(self) -> list:
        """Извлекает действия из вывода модели"""
        return self.orchestrator.state.last_model_output.action
    ...
    async def finalize_step_processing(self) -> None:
        ...
        action_results = self.orchestrator.state.last_result
        ...
        if action_results and len(action_results) == 1 and action_results[-1].error:
            ...
            self.orchestrator.logger.debug(f'🔄 Шаг {self.orchestrator.state.n_steps}: Последовательные неудачи: {self.orchestrator.state.consecutive_failures}')
            # Проверка загрузок даже при ошибке
            await self.orchestrator._check_and_update_downloads('после выполнения действий')
            return

        # Сброс счетчика неудач при успехе
        if self.orchestrator.state.consecutive_failures > 0:
            self.orchestrator.state.consecutive_failures = 0
            self.orchestrator.logger.debug(f'🔄 Шаг {self.orchestrator.state.n_steps}: Последовательные неудачи сброшены до: {self.orchestrator.state.consecutive_failures}')
        ...
        if action_results and len(action_results) > 0 and action_results[-1].is_done:
            ...
            if final_result.attachments:
                ...
                for i, file_path in enumerate(final_result.attachments):
                    self.orchestrator.logger.info(f'👉 Attachment {i + 1 if total_attachments > 1 else ""}: {file_path}')
    ...
    async def make_history_item(
        ...
        if agent_decision:
            dom_state = self._get_page_state_attr(page_state, 'dom_state')
            selector_map = self._get_page_state_attr(dom_state, 'selector_map', {}) if dom_state else {}
            interacted_elements = ExecutionHistory.get_interacted_element(agent_decision, selector_map)
        else:
        ...
        if screenshot:
            self.orchestrator.logger.debug(
                f'📸 Storing screenshot for step {self.orchestrator.state.n_steps}, screenshot length: {len(screenshot)}'
            )
            screenshot_path = await self.orchestrator.screenshot_service.store_screenshot(screenshot, self.orchestrator.state.n_steps)
            self.orchestrator.logger.debug(f'📸 Screenshot stored at: {screenshot_path}')
        else:
            self.orchestrator.logger.debug(f'📸 No screenshot in page_state for step {self.orchestrator.state.n_steps}')
    ...
    async def handle_post_llm_processing(
        ...
        agent_decision = self.orchestrator.state.last_model_output
        if self.orchestrator.register_new_step_callback and agent_decision:
            if inspect.iscoroutinefunction(self.orchestrator.register_new_step_callback):
                await self.orchestrator.register_new_step_callback(
                    page_state,
                    agent_decision,
                    self.orchestrator.state.n_steps,
                )
            else:
                self.orchestrator.register_new_step_callback(
                    page_state,
                    agent_decision,
                    self.orchestrator.state.n_steps,
                )

        if self.orchestrator.settings.save_conversation_path and agent_decision:
            # Обработка save_conversation_path как директории (согласованно с другими путями записи)
            conversation_dir = Path(self.orchestrator.settings.save_conversation_path)
            conversation_filename = f'conversation_{self.orchestrator.id}_{self.orchestrator.state.n_steps}.txt'
            target = conversation_dir / conversation_filename
            await save_conversation(
                context_messages,
                agent_decision,
                target,
                self.orchestrator.settings.save_conversation_path_encoding,
            )
    ...
    def detect_variables(self) -> dict[str, DetectedVariable]:
        ...
        return detect_variables_in_history(self.orchestrator.history)

    def save_history(self, file_path: str | Path | None = None) -> None:
        """Save the history to a file with sensitive data filtering"""
        if not file_path:
            file_path = 'ExecutionHistory.json'
        self.orchestrator.history.save_to_file(file_path, sensitive_data=self.orchestrator.sensitive_data)
    ...
    async def finalize(
        ...
        if page_state:
            ...
            if len(self.orchestrator.history.history) > 0:
                last_history_item = self.orchestrator.history.history[-1]
            ...
            metadata = StepMetadata(
                step_number=self.orchestrator.state.n_steps,
            ...
            await self.make_history_item(
                ...
                action_results,
                metadata,
                state_message=self.orchestrator._message_manager.last_state_message_text,
            )

        # Логирование сводки завершения шага
        summary_message = self.orchestrator._run_manager.log_step_completion_summary(step_start_time, action_results)
        if summary_message:
            await self.orchestrator._run_manager.demo_mode_log(summary_message, 'info', {'step': self.orchestrator.state.n_steps})

        # Сохранение состояния файловой системы после завершения шага
        self.orchestrator.save_file_system_state()
        ...
        self.orchestrator.state.n_steps += 1

    def _make_history_item_with_error(self, error: str):
        """Create a history item with an error."""
        return ExecutionHistory(
            model_output=None,
            result=[ExecutionResult(error=error, include_in_memory=True)],
