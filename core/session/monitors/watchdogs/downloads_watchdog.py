"""Watchdog для мониторинга и обработки загрузок файлов."""
...
class DownloadsWatchdog(BaseWatchdog):
	"""Мониторит загрузки и обрабатывает события загрузки файлов."""

	# События, на которые реагирует watchdog (для документации)
	...
	# События, которые эмитирует watchdog
	EMITS: ClassVar[list[type[BaseEvent[Any]]]] = [
		FileDownloadedEvent,
	]

	# Приватное состояние
	_sessions_with_listeners: set[str] = PrivateAttr(default_factory=set)  # Сессии с установленными обработчиками загрузок
	_active_downloads: dict[str, Any] = PrivateAttr(default_factory=dict)
	_pdf_viewer_cache: dict[str, bool] = PrivateAttr(default_factory=dict)  # Кэш статуса PDF viewer по URL цели
	_download_cdp_session_setup: bool = PrivateAttr(default=False)  # Установлена ли CDP сессия
	_download_cdp_session: Any = PrivateAttr(default=None)  # Ссылка на CDP сессию
	_cdp_event_tasks: set[asyncio.Task] = PrivateAttr(default_factory=set)  # Задачи обработки CDP событий
	_cdp_downloads_info: dict[str, dict[str, Any]] = PrivateAttr(default_factory=dict)  # Маппинг guid -> info
	_use_js_fetch_for_local: bool = PrivateAttr(default=False)  # Защита JS fetch пути для локальных регулярных загрузок
	_session_pdf_urls: dict[str, str] = PrivateAttr(default_factory=dict)  # URL -> путь для PDF, загруженных в этой сессии
	_network_monitored_targets: set[str] = PrivateAttr(default_factory=set)  # Цели с включенным мониторингом сети
	_detected_downloads: set[str] = PrivateAttr(default_factory=set)  # Обнаруженные URL загрузок для избежания дубликатов
	_network_callback_registered: bool = PrivateAttr(default=False)  # Зарегистрирован ли глобальный сетевой callback
	...
	async def on_BrowserLaunchEvent(self, event: BrowserLaunchEvent) -> None:
		...
		if downloads_dir:
			resolved_path = Path(downloads_dir).expanduser().resolve()
			resolved_path.mkdir(parents=True, exist_ok=True)
			self.logger.debug(f'[DownloadsWatchdog] Ensured downloads directory exists: {resolved_path}')

	async def on_TabCreatedEvent(self, event: TabCreatedEvent) -> None:
		"""Мониторить новые вкладки на загрузки."""
		# logger.info(f'[DownloadsWatchdog] TabCreatedEvent received for tab {event.target_id[-4:]}: {event.url}')

		# Проверить, что путь загрузок настроен (должен быть установлен по умолчанию в BrowserProfile)
	...
	async def on_TabClosedEvent(self, event: TabClosedEvent) -> None:
		"""Остановить мониторинг закрытых вкладок."""
		pass  # Очистка не требуется, браузерный контекст обрабатывает жизненный цикл цели

	async def on_BrowserStateRequestEvent(self, event: BrowserStateRequestEvent) -> None:
		"""Обработать события запроса состояния браузера."""
		# Использовать публичный API - автоматически проверяет и ждет восстановления при необходимости
		...
		try:
			...
			return  # Нет валидного фокуса, пропустить

		self.logger.debug(
			f'[DownloadsWatchdog] About to call get_current_page_url(), target_id={cdp_connection.target_id[-4:] if cdp_connection.target_id else "None"}'
		)
		current_url = await self.browser_session.get_current_page_url()
		self.logger.debug(f'[DownloadsWatchdog] Got URL: {current_url[:80] if current_url else "None"}')

		if not current_url:
			self.logger.warning(f'[DownloadsWatchdog] No URL found for BrowserStateRequestEvent {event.event_id[-4:]}')
			return

		target_identifier = cdp_connection.target_id
		self.logger.debug(f'[DownloadsWatchdog] About to dispatch NavigationCompleteEvent for target {target_identifier[-4:]}')
	...
	async def on_BrowserStoppedEvent(self, event: BrowserStoppedEvent) -> None:
		"""Очистить при остановке браузера."""
		# Отменить все задачи обработки CDP событий
		for handler_task in list(self._cdp_event_tasks):
			if not handler_task.done():
				handler_task.cancel()
		# Дождаться отмены всех задач
	...
	async def on_NavigationCompleteEvent(self, event: NavigationCompleteEvent) -> None:
		"""Проверить PDF после завершения навигации."""
		self.logger.debug(f'[DownloadsWatchdog] NavigationCompleteEvent received for {event.url}, tab #{event.target_id[-4:]}')

		# Очистить кэш PDF для навигированного URL, так как содержимое могло измениться
		if event.url in self._pdf_viewer_cache:
			del self._pdf_viewer_cache[event.url]

		# Проверить, включена ли автоматическая загрузка
		is_auto_download = self._is_auto_download_enabled()
		if not is_auto_download:
			return

		# Примечание: Используется сетевое обнаружение PDF, не требующее JavaScript

		target_identifier = event.target_id
		self.logger.debug(f'[DownloadsWatchdog] Got target_id={target_identifier} for tab #{event.target_id[-4:]}')

		pdf_detected = await self.check_for_pdf_viewer(target_identifier)

		if pdf_detected:
			self.logger.debug(f'[DownloadsWatchdog] 📄 PDF detected at {event.url}, triggering auto-download...')
			save_location = await self.trigger_pdf_download(target_identifier)
			if not save_location:
				self.logger.warning(f'[DownloadsWatchdog] ⚠️ PDF download failed for {event.url}')

	def _is_auto_download_enabled(self) -> bool:
		"""Проверить, включена ли автоматическая загрузка PDF в профиле браузера."""
		return self.browser_session.browser_profile.auto_download_pdfs

	async def attach_to_target(self, target_id: TargetID) -> None:
		"""Настроить мониторинг загрузок для конкретной цели."""

		# Определить обработчики CDP событий вне try, чтобы избежать проблем с отступами/областью видимости
		def download_will_begin_handler(event: DownloadWillBeginEvent, session_id: SessionID | None) -> None:
			...
			# Кэшировать информацию для последующей обработки события завершения (особенно для удаленных браузеров)
			download_id = event.get('guid', '')
			try:
				filename = event.get('suggestedFilename')
				assert filename, 'CDP DownloadWillBegin missing suggestedFilename'
				self._cdp_downloads_info[download_id] = {
					'url': event.get('url', ''),
			...
			handler_task = create_task_with_error_handling(
				self._handle_cdp_download(event, target_id, session_id),
				name='handle_cdp_download',
				logger_instance=self.logger,
				suppress_exceptions=True,
			)
			self._cdp_event_tasks.add(handler_task)
			# Удалить из множества при завершении
			handler_task.add_done_callback(lambda t: self._cdp_event_tasks.discard(t))
	...
	def _track_download(self, file_path: str) -> None:
		...
		try:
			...
			if path.exists():
				...
				from core.session.events import FileDownloadedEvent
	...
	async def _handle_cdp_download(
		...
		downloads_dir = (
			Path(
				...
				or f'{tempfile.gettempdir()}/agent_downloads.{str(self.browser_session.id)[-4:]}'
			)
			.expanduser()
			.resolve()
