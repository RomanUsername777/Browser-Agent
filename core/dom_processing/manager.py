...
from core.dom_processing.enhanced_snapshot import (
...
class AXNodeBuilder:
	"""Класс для построения расширенных AX узлов из базовых AX узлов."""
	
	def build(self, ax_node: AXNode) -> EnhancedAXNode:
		"""Строит расширенный AX узел из базового AX узла."""
		...
		if 'properties' in ax_node and ax_node['properties']:
			...
			for property in ax_node['properties']:
				try:
					# Проверяем, может ли имя свойства быть в enum (иногда Chrome возвращает случайные свойства)
					properties.append(
						EnhancedAXProperty(
							name=property['name'],
...
class ViewportCalculator:
	...
	async def calculate_ratio(self, target_id: TargetID) -> float:
		"""Получить размеры viewport, device pixel ratio и позицию прокрутки используя CDP."""
		cdp_session = await self.browser_session.get_or_create_cdp_session(target_id=target_id, focus=False)

		try:
			# Получить метрики layout, которые включают визуальный viewport
			metrics = await cdp_session.cdp_client.send.Page.getLayoutMetrics(session_id=cdp_session.session_id)

			visual_viewport = metrics.get('visualViewport', {})

			# ВАЖНО: Использовать CSS viewport вместо device pixel viewport
			# Исправление несоответствия координат на дисплеях с высоким DPI
			css_visual_viewport = metrics.get('cssVisualViewport', {})
			css_layout_viewport = metrics.get('cssLayoutViewport', {})

			# Использовать CSS пиксели (то, что видит JavaScript) вместо device пикселей
			width = css_visual_viewport.get('clientWidth', css_layout_viewport.get('clientWidth', 1920.0))

			# Вычислить device pixel ratio
			...
			self.logger.debug(f'Определение размера viewport не удалось: {e}')
			# Возврат к размеру viewport по умолчанию
			return 1.0
...
class DomService:
	...
	def __init__(
		...
		# Инициализация компонентов (новый паттерн композиции)
		self.ax_node_builder = AXNodeBuilder()
		self.viewport_calculator = ViewportCalculator(browser_session, logger)
	...
	def _build_enhanced_ax_node(self, ax_node: AXNode) -> EnhancedAXNode:
		"""Строит расширенный AX узел (делегирует в AXNodeBuilder)"""
		return self.ax_node_builder.build(ax_node)

	async def _get_viewport_ratio(self, target_id: TargetID) -> float:
		"""Получить viewport ratio (делегирует в ViewportCalculator)"""
		return await self.viewport_calculator.calculate_ratio(target_id)
	...
	async def _get_ax_tree_for_all_frames(self, target_id: TargetID) -> GetFullAXTreeReturns:
		...
		cdp_session = await self.browser_session.get_or_create_cdp_session(target_id=target_id, focus=False)
		frame_tree_response = await cdp_session.cdp_client.send.Page.getFrameTree(session_id=cdp_session.session_id)

		# Extract frame IDs using iterative approach instead of recursive
		def extract_frame_identifiers(root_frame_node) -> list[str]:
			"""Extract all frame identifiers from the frame tree structure."""
			collected_ids = []
			stack = [root_frame_node]
			
			while stack:
				current = stack.pop()
				if 'frame' in current and 'id' in current['frame']:
					collected_ids.append(current['frame']['id'])
				
				# Process child frames
				if 'childFrames' in current and current['childFrames']:
					stack.extend(current['childFrames'])
			
			return collected_ids

		# Gather all frame identifiers
		frame_id_list = extract_frame_identifiers(frame_tree_response['frameTree'])

		# Prepare accessibility tree requests for all frames
		accessibility_requests = []
		for fid in frame_id_list:
			req = cdp_session.cdp_client.send.Accessibility.getFullAXTree(
				params={'frameId': fid}, session_id=cdp_session.session_id
			)
			accessibility_requests.append(req)

		# Execute all requests concurrently, allowing exceptions to be returned
		# This prevents crashes when frames disappear during page reloads
		accessibility_results = await asyncio.gather(*accessibility_requests, return_exceptions=True)

		# Combine all accessibility nodes from successful requests
		combined_ax_nodes: list[AXNode] = []
		for result_idx, result in enumerate(accessibility_results):
			if isinstance(result, Exception):
				# Log as debug since frame disappearance is expected during page reloads
				frame_id_str = frame_id_list[result_idx] if result_idx < len(frame_id_list) else "unknown"
				self.logger.debug(f'Failed to get AX tree for frame {frame_id_str}: {result}')
				continue
			if result and isinstance(result, dict) and 'nodes' in result:
				combined_ax_nodes.extend(result['nodes'])

		return {'nodes': combined_ax_nodes}

	async def _get_all_trees(self, target_id: TargetID) -> TargetAllTrees:
		cdp_session = await self.browser_session.get_or_create_cdp_session(target_id=target_id, focus=False)

		# Check page readiness (non-blocking)
		try:
			await cdp_session.cdp_client.send.Runtime.evaluate(
				params={'expression': 'document.readyState'}, session_id=cdp_session.session_id
			)
		except Exception:
			pass  # Page might not be ready yet, continue anyway

		self.logger.debug(f'🔍 DEBUG: Capturing DOM snapshot for target {target_id}')

		# Collect iframe scroll positions before snapshot capture
		iframe_scroll_timer_start = time.time()
		iframe_scroll_data = {}
		try:
			scroll_eval_result = await cdp_session.cdp_client.send.Runtime.evaluate(
				params={
					'expression': """
					(() => {
						const scrollData = {};
			...
			if scroll_eval_result and 'result' in scroll_eval_result and 'value' in scroll_eval_result['result']:
				iframe_scroll_data = scroll_eval_result['result']['value']
				for iframe_idx, scroll_info in iframe_scroll_data.items():
					self.logger.debug(
						f'🔍 DEBUG: Iframe {iframe_idx} actual scroll position - scrollTop={scroll_info.get("scrollTop", 0)}, scrollLeft={scroll_info.get("scrollLeft", 0)}'
					)
		except Exception as e:
			self.logger.debug(f'Failed to get iframe scroll positions: {e}')
		iframe_scroll_elapsed_ms = (time.time() - iframe_scroll_timer_start) * 1000

		# Helper functions to create CDP requests
		def make_snapshot_cdp_request():
		...
		def make_dom_document_cdp_request():
			return cdp_session.cdp_client.send.DOM.getDocument(
				params={'depth': -1, 'pierce': True}, session_id=cdp_session.session_id
			)

		cdp_requests_start_time = time.time()

		# Initialize parallel CDP request tasks
		cdp_task_map = {
			'snapshot': create_task_with_error_handling(make_snapshot_cdp_request(), name='get_snapshot'),
			'dom_tree': create_task_with_error_handling(make_dom_document_cdp_request(), name='get_dom_tree'),
		...
		completed_tasks, incomplete_tasks = await asyncio.wait(cdp_task_map.values(), timeout=10.0)

		# Handle incomplete tasks with retry logic
		if incomplete_tasks:
			# Cancel incomplete tasks
			for incomplete_task in incomplete_tasks:
				incomplete_task.cancel()

			# Map tasks to retry factories
			task_retry_factories = {
				cdp_task_map['snapshot']: lambda: create_task_with_error_handling(make_snapshot_cdp_request(), name='get_snapshot_retry'),
				cdp_task_map['dom_tree']: lambda: create_task_with_error_handling(make_dom_document_cdp_request(), name='get_dom_tree_retry'),
				cdp_task_map['ax_tree']: lambda: create_task_with_error_handling(
					self._get_ax_tree_for_all_frames(target_id), name='get_ax_tree_retry'
				),
				cdp_task_map['device_pixel_ratio']: lambda: create_task_with_error_handling(
					self._get_viewport_ratio(target_id), name='get_viewport_ratio_retry'
				),
			}

			# Replace incomplete tasks with retries
			for task_key, original_task in cdp_task_map.items():
				if original_task in incomplete_tasks and original_task in task_retry_factories:
					cdp_task_map[task_key] = task_retry_factories[original_task]()

			# Wait again with reduced timeout
			completed_retry, incomplete_retry = await asyncio.wait(
				[t for t in cdp_task_map.values() if not t.done()], timeout=2.0
			)

			if incomplete_retry:
				for task in incomplete_retry:
					task.cancel()

		# Process task results
		cdp_results = {}
		failed_request_keys = []
		for request_key, request_task in cdp_task_map.items():
			if request_task.done() and not request_task.cancelled():
				try:
					cdp_results[request_key] = request_task.result()
				except Exception as e:
					# Expected when page reloads or frame disappears
					self.logger.debug(f'CDP request {request_key} failed with exception: {e}')
					failed_request_keys.append(request_key)
			else:
				self.logger.debug(f'CDP request {request_key} timed out')
				failed_request_keys.append(request_key)

		# Validate all required requests succeeded
		if failed_request_keys:
			raise TimeoutError(f'CDP requests failed or timed out: {", ".join(failed_request_keys)}')

		# Extract individual results
		dom_snapshot = cdp_results['snapshot']
		dom_document_tree = cdp_results['dom_tree']
		accessibility_tree = cdp_results['ax_tree']
		pixel_ratio = cdp_results['device_pixel_ratio']
		cdp_requests_end_time = time.time()
		cdp_requests_duration_ms = (cdp_requests_end_time - cdp_requests_start_time) * 1000

		# Process snapshot data
		snapshot_processing_start = time.time()

		# Limit iframe documents to prevent explosion
		if dom_snapshot and 'documents' in dom_snapshot:
			initial_document_count = len(dom_snapshot['documents'])
			if initial_document_count > self.max_iframes:
				self.logger.warning(
					f'⚠️ Limiting processing of {initial_document_count} iframes on page to only first {self.max_iframes} to prevent crashes!'
				)
				dom_snapshot['documents'] = dom_snapshot['documents'][: self.max_iframes]

			aggregate_node_count = sum(len(doc.get('nodes', [])) for doc in dom_snapshot['documents'])
			self.logger.debug(f'🔍 DEBUG: Snapshot contains {len(dom_snapshot["documents"])} frames with {aggregate_node_count} total nodes')
			# Log iframe-specific info
			for document_index, document_data in enumerate(dom_snapshot['documents']):
				if document_index > 0:  # Not the main document
					self.logger.debug(
						f'🔍 DEBUG: Iframe #{document_index} {document_data.get("frameId", "no-frame-id")} {document_data.get("url", "no-url")} has {len(document_data.get("nodes", []))} nodes'
					)

		snapshot_processing_duration_ms = (time.time() - snapshot_processing_start) * 1000

		# Return results with timing information
		return TargetAllTrees(
			snapshot=dom_snapshot,
			dom_tree=dom_document_tree,
			ax_tree=accessibility_tree,
			device_pixel_ratio=pixel_ratio,
			cdp_timing={
				'iframe_scroll_detection_ms': iframe_scroll_elapsed_ms,
				'cdp_parallel_calls_ms': cdp_requests_duration_ms,
				'snapshot_processing_ms': snapshot_processing_duration_ms,
			},
		)
	...
	async def get_dom_tree(
		...
		# Fetch all required trees from CDP (snapshot, DOM, AX, viewport ratio)
		trees_fetch_start = time.time()
		all_trees_data = await self._get_all_trees(target_id)
		trees_fetch_duration_ms = (time.time() - trees_fetch_start) * 1000
		timing_info.update(all_trees_data.cdp_timing)
		timing_info['get_all_trees_total_ms'] = trees_fetch_duration_ms

		# Extract individual tree components
		dom_document_tree = all_trees_data.dom_tree
		accessibility_tree_data = all_trees_data.ax_tree
		dom_snapshot_data = all_trees_data.snapshot
		viewport_pixel_ratio = all_trees_data.device_pixel_ratio

		# Construct accessibility tree lookup dictionary
		ax_lookup_start = time.time()
		accessibility_lookup: dict[int, AXNode] = {}
		for ax_node_item in accessibility_tree_data['nodes']:
			if 'backendDOMNodeId' in ax_node_item:
				accessibility_lookup[ax_node_item['backendDOMNodeId']] = ax_node_item
		timing_info['build_ax_lookup_ms'] = (time.time() - ax_lookup_start) * 1000

		# Node lookup cache: maps nodeId (NOT backend node id) to enhanced dom tree node
		enhanced_dom_tree_node_lookup: dict[int, EnhancedDOMTreeNode] = {}

		# Build snapshot lookup with pre-calculated data
		snapshot_lookup_start = time.time()
		snapshot_lookup = build_snapshot_lookup(dom_snapshot_data, viewport_pixel_ratio)
		timing_info['build_snapshot_lookup_ms'] = (time.time() - snapshot_lookup_start) * 1000
		...
		async def _construct_enhanced_node(
			...
			# Look up corresponding accessibility node
			corresponding_ax_node = accessibility_lookup.get(node['backendNodeId'])
			if corresponding_ax_node:
				enhanced_ax_node = self._build_enhanced_ax_node(corresponding_ax_node)
			else:
		...
		# Recursively construct enhanced DOM tree
		# Note: all_frames remains None and will be lazily fetched inside _construct_enhanced_node
		# only when a cross-origin iframe is actually encountered
		tree_construction_start = time.time()
		root_enhanced_node = await _construct_enhanced_node(
			dom_document_tree['root'], initial_html_frames, initial_total_frame_offset, all_frames
		)
		timing_info['construct_enhanced_tree_ms'] = (time.time() - tree_construction_start) * 1000

		# Compute total execution time
		total_execution_time_ms = (time.time() - timing_start_total) * 1000
		timing_info['get_dom_tree_total_ms'] = total_execution_time_ms

		# Compute overhead (time not accounted for by tracked sub-operations)
		sum_of_tracked_operations_ms = (
		...
		overhead_time_ms = total_execution_time_ms - sum_of_tracked_operations_ms
		if overhead_time_ms > 0.1:
			timing_info['get_dom_tree_overhead_ms'] = overhead_time_ms

		return root_enhanced_node, timing_info
